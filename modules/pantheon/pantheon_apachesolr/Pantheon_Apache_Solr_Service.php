...
class Pantheon_Apache_Solr_Service extends Apache_Solr_Service {
  ...
  public function getFields($num_terms = 0) {
    $facets = variable_get('apachesolr_enabled_facets', array());
  ...
  public function getLuke($num_terms = 0) {
    $luke = array(
      'index' => array(
        'numDocs' => db_result(db_query("SELECT TABLE_ROWS FROM information_schema.tables WHERE TABLE_SCHEMA = '%s'  AND TABLE_NAME = '{node}'", $_SERVER['db_name'])),
      ),
    );
    return (object) $luke;
  }
